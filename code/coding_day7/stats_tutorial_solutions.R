...
ggplot(data = data1) + 
  ...
  theme_classic() +
  labs(title = "Growth in Trials 2 and 3 for 1x Iron" )
...
ggplot(data = Trial2) + 
  ...
  theme_classic() +
  labs(title = "Growth in Trials 2 for 1x Iron" )
...
predicted_df <- data.frame(predicted_OD = predict(model, Trial2), 
                           Hours = Trial2$Hours)
...
ggplot(data = Trial2) + 
  ...
  theme_classic() +
  labs(title = "Growth in Trials 2 for 1x Iron") +
  geom_line(mapping = aes(x= Hours, y= exp(predicted_OD)), 
            data = predicted_df, 
            color = 'red' )
