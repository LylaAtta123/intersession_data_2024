...
plt3 <- ggplot(data = mpg) +
  ...
  scale_color_viridis() +
  labs(title = "A", x = "mpg in city", y = "mpg on hwy", 
       #subtitle = "this is a subtitle",
       # tag = "A",
       #caption = "A: Mpg in different driving scenario by # of cylinders"
       )
...
plt4 <- ggplot(data = mpg) + 
  ...
  labs(title = "B", x = "mpg in city", y = "mpg on hwy", 
       #subtitle = "this is a subtitle",
       #tag = "B",
       #caption = "B: Mpg in different driving scenario by type of car"
       )
...
# saving printout
png('vehicle_characteristics_caption.png', width = 10, height = 5, units = "in", res = 400)
grid.arrange(plt3, plt4, ncol=2, nrow=1)
dev.off()
